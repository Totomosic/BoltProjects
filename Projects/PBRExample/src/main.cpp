...
public:
		...
		ResourceManager::Get().LoadPack("res/loadingResources.pack", [&uiLayer, this](const ResourcePack& pack)
			{
				...
				std::unique_ptr<Material> material = ResourceManager::Get().Materials().Texture(resources.GetResourcePtr<Texture2D>("loadingSymbol"));
				UIRectangle& loadingSymbol = uiLayer.UI().Root().CreateRectangle(200, 200, std::move(material), Transform({ Width() / 2, Height() / 2, -10 }));
				loadingSymbol.GetGameObject()->mesh().Mesh.Materials[0]->SetIsTransparent(true);
				loadingSymbol.GetGameObject()->Components().AddComponent<TriggerComponent>(TriggerComponent::TriggerFunc(), [](GameObject* object)
					{
	...
	void Tick() override
	{
		GetWindow().SetTitle(std::to_string(Time::Get().FramesPerSecond()));
	}
