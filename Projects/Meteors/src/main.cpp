...
namespace Meteors
{
	...
	private:
		...
		UIText* m_TimerText = nullptr;

		UIText* m_FinalScoreText;
	...
	public:
		void Init() override
		{
			...
			endLayer.UI().Root().CreateText("Game Over", ResourceManager::Get().Fonts().Verdana(48), Color::Black, Transform({ ViewWidth / 2, ViewHeight / 2, 0 }));
			m_FinalScoreText = &endLayer.UI().Root().CreateText("Your final survival time was: ", ResourceManager::Get().Fonts().Verdana(30), Color::Black, Transform({ ViewWidth / 2, ViewHeight / 2 - 24 - 15, 0 }));

			UIRectangle& retryButton = endLayer.UI().Root().CreateRectangle(300, 50, Color::Green, Transform({ ViewWidth / 2, ViewHeight / 2 - 100, 0 }));
			retryButton.CreateText("Play Again", ResourceManager::Get().Fonts().Verdana(24), Color::Black, Transform({ 0, 0, 1 }));
			retryButton.Events().OnClick.AddEventListener([this](Event<UIClickedEvent>& e)
				{
					LoadGameScene();
				});
		...
		void LoadGameScene()
		{
			...
			ConstantVec2Node& scaling = groundGraph.AddNode(std::make_unique<ConstantVec2Node>(Vector2f{ 19.2f, 1.0f }));
			...
			m_TimerText = &layer.UI().Root().CreateText("0:00", Color::Black, Transform({ 10, ViewHeight - 10, 99 }), AlignH::Left, AlignV::Top);
		}
